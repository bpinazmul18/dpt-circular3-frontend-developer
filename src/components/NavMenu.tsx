...
export default function NavMenu({ menus }: NavMenuType) {
  ...
  return (
    <>
      ...
      <ul
        className={`w-full z-50 hidden lg:flex flex-col lg:flex-row justify-center items-center text-3xl gap-6 lg:text-base lg:gap-2 absolute h-screen lg:left-0 lg:top-0 lg:relative lg:h-auto lg:w-auto bg-[#1F2937] lg:bg-transparent ${openMobileMenu ? "!flex top-20 left-0" : "top-0 left-full"}`}
      >
        {menus.map((menu) => (
          <NavItem
            key={menu.id}
